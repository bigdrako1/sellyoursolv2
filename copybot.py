...
### WINNING POSITIONS FIRST
    for index, row in winning_positions_df.iterrows():
        ...
        if token_mint_address not in DO_NOT_TRADE_LIST:
            ...
            n.pnl_close_ranker(token_mint_address)
...
## NOW CHECKING LOSING POSITIONS - WE DO THIS 2nd TO CAPTURE PROFITS QUICKER
    ...
    try:
        all_trending_ever = pd.read_csv(ALL_TRENDING_EVER_CSV)
        if all_trending_ever.empty:
            all_trending_ever = pd.DataFrame(columns=df.columns)
    except (FileNotFoundError, pd.errors.EmptyDataError):
        all_trending_ever = pd.DataFrame(columns=df.columns)
    
    # Append new trending tokens
    all_trending_ever = pd.concat([all_trending_ever, df], ignore_index=True)
    
    # Remove duplicates based on 'contract_address'
    all_trending_ever = all_trending_ever.drop_duplicates(subset=['contract_address'], keep='last')
    
    # Save updated ALL_TRENDING_EVER
    all_trending_ever.to_csv(ALL_TRENDING_EVER_CSV, index=False)
    
    print(f"Total unique trending tokens ever: {len(all_trending_ever)}")
